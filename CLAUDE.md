...
- **[TODO-12] 哨表自動化工作流** — 🟢 開發完成，待部署與人工驗收。`tool_guard_gen.html`（哨點設定＋產生哨表兩分頁，React 18.3.1）＋ `哨表產生_GAS.gs`（讀早/晚班月班表算當日在職名單、讀寫哨點設定分頁）；產生邏輯（隨機分配崗位、巡檢/停巡/泊車增派、見習自動歸位）已用 node 測試 16 項斷言全過。上傳沿用既有 `哨表上傳_GAS_v6.gs` 的 `guardUpload` action。部署步驟見 `哨表產生_GAS_部署說明.md`；部署後把 `/exec` 網址回填 `tool_guard_gen.html` 的 `GUARD_GEN_GAS_URL`。帶隊幹部無自動規則，改為手動輸入欄位（規格未定義自動分配依據）。

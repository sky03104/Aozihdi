...
- **狀態**：🟡 部分完成（2026-07-11 22:45 跑過一輪，查出 index.html 內嵌 React19.2.5，根因見 `docs/技術經驗筆記.md` 2026-07-11 段，決定暫不降版）。7/11 健檢**之後**新增/修改的部分（7/12 哨表與車牌辨識修復、吉祥物系統擴充）尚未被覆蓋到，效能／資安面向也還沒跑，待補一輪範圍限定的健檢
...
- **現況盤點（2026-07-13 查 git 歷史＋`chars/`資料夾核實）**：
  - 已達4段 ✅：咖波、白爛貓
  - 3段（技術卡關）：鯊魚先生——背景雨滴線條跟角色輪廓太近，去背時被形態學閉運算橋接黏在一起，當時直接放棄第4段，見 `docs/技術經驗筆記.md` 2026-07-11 段
  - 1段待擴充：情侶貓、愛心小狐、奔騰小馬、小呆雞（新增時都只做了1個循環，之後沒再擴充過）
...
- **狀態**：🟡 待開發，逐隻角色慢慢補，不用一次做完。建議順序：先補1段→4段的4隻（情侶貓/愛心小狐/奔騰小馬/小呆雞），鯊魚先生第4段跟5隻海賊王靜態立繪視情況再議

#### [TODO-29] 咖哩海域跑步動畫補完（brain_map.html RUN_IMG_MAP，跟TODO-28是不同系統）
- 目標：`brain_map.html`（咖哩海域3D知識圖譜）節點角色的跑步循環動畫，咖哩上傳12支角色的12格跑步姿勢來源圖到 `brain_map_img/run_<英文名>.png`，比照既有路飛/羅賓機制補完
- **2026-07-13 已完成6隻**：索隆(12幀)/娜美(12幀)/喬巴(12幀)/山治(12幀)/烏索普(12幀)/羅(15幀)，已寫入 `RUN_IMG_MAP` 並驗證載入成功，完整處理過程見 `docs/技術經驗筆記.md` 2026-07-13 段
- **剩餘6隻卡在來源圖品質，需要咖哩決定怎麼處理**：
  - 布魯克、弗蘭奇——6x2格但缺一格、每格下方燒進數字標籤圖層，需要額外的去標籤裁切邏輯
  - 艾斯、克比、甚平、佩羅娜——來源圖背景帶橘/白暈光漸層（非純黑）、角色彼此重疊沒有乾淨格線縫，現有去背算法處理不了
- **狀態**：🟡 部分完成，剩餘6隻建議咖哩重新出圖（純黑背景、角色間留乾淨空隙、不要燒數字標籤進圖層）會最快，或接受人工逐格裁切的較高成本

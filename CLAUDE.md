...
- **[TODO-36] 班表管理搬遷 SQL（Supabase）** — 🟡 規劃完成，開始執行（2026-08-27），分支 `claude/schedule-sql-migration`，完整規劃見 `docs/SQL遷移規劃_班表管理.md`。**動機**：班表管理是全公司最常用工具，讀取效能最重要；且現有「換月自動備份」機制（TODO-31）複製整分頁當備份、只增不刪，長期會撐爆 Google Sheets 分頁數上限。**做法**：GAS 改用 `UrlFetchApp` 呼叫 Supabase REST API 取代讀寫 Sheets，前端網址與回傳格式不變、index.html/tool_billing.html/tool_guard_gen.html **不需要改動**。分六階段（建置→歷史資料搬遷→只換讀→換寫→觀察期→收尾），每階段有明確驗收條件，確認沒問題才合併回 main。順便修掉「休假用紅字色判斷」這個脆弱的視覺語意耦合，改存明確代號。

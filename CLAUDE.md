...
- **[TODO-40] 物流車輛統計搬遷 SQL（Supabase）** — 🟡 規劃中（2026-08-27開始），完整規劃見 `docs/SQL遷移規劃_物流車輛統計.md`。**範圍**：只搬「物流車輛紀錄」（每日持續累積、無上限成長，同一種效能債），「快捷設定」量小固定不搬。**比打烊/開店單純**：`exportMonth`只是把彙總結果寫進試算表新分頁，不是Sheets原生xlsx匯出、也沒有公式即時監控畫面依賴，不需要「按需刷新」那層設計。**已完成**：階段1建表SQL（`docs/物流車輛統計SQL建表.sql`）、階段2搬遷腳本（`物流車輛_SQL遷移腳本.gs`，沿用打烊/開店踩過的坑不用legacy_id當唯一鍵防重複，改執行前檢查Supabase是否已有資料）、階段3讀取層＋比對/效能測試工具（`物流車輛_SQL讀取層.gs`）皆已寫好待部署，node --check皆通過。**待咖哩手動操作**：GAS的Script Properties設定`SUPABASE_URL`/`SUPABASE_SECRET_KEY`、貼上對應腳本執行遷移＋比對+效能測試，確認結果後才繼續階段4（換寫邏輯）。

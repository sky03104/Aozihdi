...
- **[TODO-37] 施工單管理搬遷 SQL（Supabase）** — 🟡 規劃完成，開始執行（2026-08-27），分支 `claude/construction-sql-migration`，完整規劃見 `docs/SQL遷移規劃_施工單管理.md`。**動機**：跟TODO-36不同，這裡是實測證實的效能債——每天新增幾十筆、歷史從不清理，ID產生/去重/報到換證都是全表掃描，報到換證還是高頻動作。歷史上已因「寫入去重」跟「顯示去重」兩套邏輯對不上出過至少3次資料遺失/重複bug。**做法**：現場確認只透過工具寫入、沒有人直接編輯Sheets，Supabase可以直接當真正權威來源（比TODO-36更進一步，不用像班表管理保留Sheets當永久權威）。分六階段，用資料庫唯一鍵一次解決去重bug根因。⚠️ `tool_work.html`用gviz無驗證讀取是刻意設計（給指定電腦免登入使用，非漏洞），搬遷時要維持免登入特性，不能加驗證。

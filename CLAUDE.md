...
- **[TODO-39] 打烊/開店管理搬遷 SQL（Supabase）** — 🟡 規劃中（2026-08-27開始），完整規劃見 `docs/SQL遷移規劃_打烊開店管理.md`。**範圍**：只搬「進出資料表」（每日持續累積、無上限成長，跟施工單管理同一種效能債），**專櫃表／_SharedDB 刻意不搬**（筆數受早晚班共用的實體專櫃/樓層數量限制、不會隨時間增長，比照班表管理`getSchedule`的教訓，範圍固定小的資料改Supabase反而更慢）。**獨有難點**：`exportDailyExcel`每日排程依賴Sheets原生匯出API，Supabase當權威來源後改用「建暫存分頁貼資料→匯出→刪除」的方式產生xlsx，行為對使用者不變。打烊/開店是兩支獨立試算表＋獨立GAS，各建一張表（`closing_gate_logs`/`opening_gate_logs`），不合併。**已完成**：階段1建表SQL（`docs/打烊開店SQL建表.sql`）、階段2搬遷腳本（`打烊_SQL遷移腳本.gs`/`開店_SQL遷移腳本.gs`）、階段3讀取層＋比對/效能測試工具（`打烊_SQL讀取層.gs`/`開店_SQL讀取層.gs`）皆已寫好待部署，node --check皆通過。**待咖哩手動操作**：兩支GAS各自的Script Properties設定`SUPABASE_URL`/`SUPABASE_SECRET_KEY`、貼上對應腳本執行遷移＋比對+效能測試，確認結果後才繼續階段4（換寫＋改匯出邏輯）。

...
- **2026-07-13 已完成11隻**：索隆/娜美/喬巴/山治/烏索普(各12幀)/羅(15幀)，及咖哩重新出圖後補上的艾斯/克比/甚平/佩羅娜/布魯克(各12幀)，已寫入 `RUN_IMG_MAP` 並驗證載入成功，完整處理過程見 `docs/技術經驗筆記.md` 2026-07-13 段
- **僅剩弗蘭奇**：來源圖(`run_franky.png`)6x2格但缺一格、每格下方燒進數字標籤圖層，咖哩尚未重新上傳，其餘5隻同樣問題已靠重新出圖解決
- **狀態**：🟡 只差弗蘭奇一隻，等咖哩重新出圖（純黑或純白背景皆可、角色間留乾淨空隙、不要燒數字標籤進圖層）後即可比照同一套流程補上

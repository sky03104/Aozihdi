...
- **狀態**：🟡 部分完成（最後核實 2026-07-29）。7/11 22:45 跑過一輪主動健檢，查出 index.html 內嵌 React19.2.5（根因見 `docs/技術經驗筆記.md` 2026-07-11 段，決定暫不降版）。7/11 之後**資安／效能面已有大量進展**，但多數是個別踩坑修復、不是系統性健檢：
  ...
  - **功能正確性**（2026-07-29）：13 支工具＋index.html 主控台核心全數複查完畢，13 支全部有發現問題（7 項 🔴 阻擋、多項 🟡），完整清單見 `docs/健檢報告_2026-07-29_功能正確性.md`。咖哩已指定優先修 #1（請假早/晚班配額失真）#3（班表上傳內容防呆遺失）#5（自動排哨同人雙崗無警告）#6（事故/表揚日期跨夜算錯）四項，**已修復並經獨立 code-reviewer 驗收 PASS**（0🔴，2🟡收斂項不影響交付，詳見報告）。尚未修：#2 打烊登錄今日分頁編輯/刪除100%失敗、#4 施工單查詢三個資料遺失/重複問題、#7 過夜車輛登記無防連點/無empId驗證，以及一批 empId 未驗證的中等問題（列在報告 🟡 段）
  - 待評估：三大面向（資安／效能／功能正確性）都已跑過一輪，🔴 清單剩 #2 #4 #7 三項未修——下次動工前先問咖哩要不要接著修，或關閉 TODO-22 另開新待辦追蹤

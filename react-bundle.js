// 天鷹保全 APP — React 19.2.5 執行環境（含 React 與 React-DOM）
// 2026-07-26 從 index.html 抽出獨立成檔，程式碼內容完全未變動。
// 原本整包塞在 index.html 裡（約 1.7 萬行），是本檔過度肥大、
// AI 助手難以安全編輯的主因之一；抽成獨立檔後瀏覽器也能對它單獨快取，
// 不必每次 index.html 更新都重新下載這一大包。
//
// 版本仍是 19.2.5（沿用原狀，未做版本升降級）：React 19 官方已不提供
// CDN 可直接載入的 UMD 版本，因此無法簡單改成 <script src="unpkg.com/react@19...">
// 的寫法，只能像現在這樣把原始碼整包收在同一個檔案裡。
// 是否降版到專案規範要求的 18.3.1，是另一個需要重新驗證整個主控台
// 功能的較大決定，見 docs/技術經驗筆記.md 2026-07-11 段，本次未處理。
...
exports.version = "19.2.5";
  ...
  /* ── 掛載到 window ── */
  global.React    = __require('react');
  global.ReactDOM = __require('react-dom/client');

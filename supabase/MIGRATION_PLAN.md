...
- [x] **Phase 6：Edge Function 已部署**（`functions/admin-users/index.ts`）——
      處理「核准申請→建帳號」／「管理員新增帳號」／「管理員重設密碼」，
      這是唯一需要 service role key 的地方。2026-08-29 用 Supabase MCP
      `deploy_edge_function` 部署完成，`verify_jwt: true`，狀態 ACTIVE，
      URL `https://tjrlpthprtrlmugrofpj.supabase.co/functions/v1/admin-users`，
      跟前端 `callAdminUsersFn()` 呼叫路徑核對一致。**還沒實測**。

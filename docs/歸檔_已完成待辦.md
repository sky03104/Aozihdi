...
#### [TODO-10 尾巴] 班表「停休加班」代號
- **狀態**：✅ 完成（2026-07-14）
- 症狀：黃底停休加班跟休假同字「休」無法區分
- 修法：咖哩決定用 `休A`／`休B` 兩個代號都代表停休加班，`index.html` 新增 `休A`（SHIFT_TYPES 定義＋baseCodes 辨識清單），內容與 `休B` 完全一致（黃底 #EADC00、label「停休加班」、12小時）
- 驗證：咖哩截圖確認——班別編輯彈窗 `休A`/`休B` 並列可選、月班表 `休B` 正確顯示黃底

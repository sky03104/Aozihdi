...
- [x] 階段2：歷史資料搬遷＋清理（2026-08-27）。施工單查詢讀到4302列全數乾淨無重複，寫入4302筆；動火申請查詢讀到194列，9筆完全重複（第32~38、129~130列，B~K十欄含日期完全一致）略過，寫入185筆。經直接查詢Supabase核對筆數完全吻合（4302/185）。咖哩確認不需要逐筆核對重複明細，同意繼續。
  - ⚠️ 踩坑記錄：`getSheetByName('施工單查詢')` 在這個既有專案裡執行失敗，丟出詭異的`Sheet 38985608 not found`（一個數字，明顯是別的試算表的gid，非分頁名稱錯誤該有的訊息），懷疑貼上過程中分頁名稱字串夾帶了看不見的特殊字元導致比對失敗，改用`getSheetId()`比對gid（用`診斷目前試算表()`先列出實際gid：施工單查詢=0、動火申請查詢=294199656）繞過後解決。**教訓：跨系統複製中文字串（尤其是拿來做字串比對用途時）不完全可信，能用不受文字差異影響的ID/gid比對就優先用ID，不要只靠名稱字串。**
  - ⚠️ 踩坑記錄（延續）：連**原本沒被這次搬遷碰過的舊版`getOrders()`**都出現一模一樣的`Sheet 38985608 not found`錯誤，代表這不是遷移改壞的，是這份試算表的分頁名稱字串比對本身就有問題（懷疑含看不見的特殊字元），可能是既有的潛在問題只是還沒被踩到。已把`getOrders()`也一併改用gid查找解決。**待辦（進phase4前要處理）**：`doPost`裡施工單/動火兩個寫入分支（約371-456行）也用`ss.getSheetByName("施工單查詢")`／`targetSs.getSheetByName("動火申請查詢")`同樣的名稱比對方式，理論上有一樣的風險，寫入路徑改造時要一併改成gid查找，不要假設寫入路徑沒事。
  - ⚠️ 補充發現：`exit_date`（M欄退場日期）在多數單日施工的既有資料裡是空白，不是每筆都有回填——這是Sheets原始資料本來的樣子，遷移時如實搬（空白存null），不擅自幫忙補值。但既有的`getOrders()`讀取邏輯在**讀取當下**會把空白的退場日期fallback成施工日期（`exitDate = row[12] instanceof Date ? d0(row[12]) : jobDate`），階段3設計Supabase讀取層時要複製這個fallback邏輯，不能只是忠實回傳null。
- [x] 階段3：只換讀＋失敗備援（2026-08-27）。`getOrders_SQL`已完成，寫入層採用區間判斷（施工日期～退場日期涵蓋今明兩天即顯示）。
  - ⚠️ 重要設計決策紀錄（2026-08-27，來回確認三次才定案）：一開始比對測試發現「天圓室內裝修」一筆新版顯示、舊版沒顯示，起初誤判成「舊版才是對的、新版邏輯有bug」，改成精確比對work_date；咖哩看過改法後確認**設計意圖其實是區間判斷（B）**——只要今天落在施工日期～退場日期區間內就該顯示，不管當天Excel分頁有沒有重新上傳那一筆。回頭查證後確認：新版顯示、舊版沒顯示，是**舊版的既有bug**（試算表該筆的日期欄位沒存成真正的日期格式，`instanceof Date`判斷失敗、意外退回精確比對，才會漏掉這筆）。最終維持區間判斷不變，新版行為視為修正了舊版的既有漏抓，非新版錯誤。**教訓：發現新舊行為不一致時，先問清楚「原本設計意圖是什麼」再判斷是誰的邏輯錯，不要看到「舊版沒顯示」就直接假設舊版才是對的——舊版本身可能就有沒被發現的bug。**
  - 範圍限定：只做`getOrders`（打烊/開店工具用）；`tool_work.html`的搜尋/歷史查詢功能需求不同（要能查任意日期、全文搜尋全部歷史），另外設計，不在這次範圍內。
  - ✅ 實際跑`比對getOrders全部模式`驗證：closing/opening兩種模式都只出現同一筆「天圓室內裝修」差異（已確認是舊版既有bug，新版行為正確），沒有其他新差異。咖哩用具體例子（8/20~8/30施工、兩種填寫習慣：整段複製貼上vs逐天手動改開始日）驗證過設計，確認現有區間判斷+模糊去重邏輯能正確處理兩種情境（複製貼上會因唯一鍵合併成1筆但區間本身涵蓋整段；逐天填寫會保留多筆但顯示時模糊去重合併，報到換證各自比對到當天那一筆）。
  - ✅ 效能實測（`測試getOrders效能`）：讀Sheets 1476ms、讀Supabase 1301ms，Supabase快約12%。目前資料量（4302筆）差距還不算大，符合預期——這個工具是持續累積不清理的資料，時間拉長後Sheets會線性變慢、Supabase靠work_date索引不太受影響，差距預期會隨資料量增加而擴大，跟班表管理（範圍固定小，SQL反而更慢）本質不同。
- [x] 階段4：換寫（2026-08-27）。上傳/手動新增/報到換證全部改雙寫Supabase（Sheets仍照常寫入，非取代——`tool_work.html`搜尋/歷史查詢功能還沒搬，仍需要Sheets持續更新）。
  - ✅ 新增資料同步驗證：實際新增一筆施工單，Sheets跟Supabase皆正確出現。
  - ⚠️ **重大踩坑（跨專案共通bug）**：報到換證同步Supabase一直失敗（HTTP 204成功但資料沒變），一路排查到發現**GAS的UrlFetchApp對大寫`'PATCH'`/`'DELETE'`方法字串會靜默忽略**（不報錯，但實際執行成別的方法或直接失效），改小寫`'patch'`/`'delete'`才真正執行。這個bug同時存在於**已經上線的班表管理專案**（換月時「標記舊版本為superseded」用的也是大寫PATCH，另開分支`claude/schedule-patch-method-fix`修復，待合併）。**教訓：GAS呼叫外部API時，HTTP方法字串一律用小寫，不要相信官方文件的大小寫範例混用（官方文件本身GET/POST常寫大寫，容易誤以為都可以）。**
  - ⚠️ 診斷過程中的假警報：一度以為報到換證「同步到錯誤的施工單列」（Supabase出現一筆廠商/項目都空白的資料被寫入報到時間），追查後發現是**我自己寫的測試函式`測試報到同步()`payload不完整**（只給ID，沒給廠商/月/日/進場時間），導致意外比對到一筆空殼列，並非正式報到換證功能的bug。已用真實UI點擊（天原室內那筆）確認正式功能運作正確，測試留下的髒資料已清除復原。**教訓：自己寫的診斷/測試函式也要小心不要用不完整的payload，避免製造出看似嚴重、實際上是測試工具自身缺陷的假警報，浪費排查時間。**
- [x] 階段5：每日備份防護（2026-08-27）。`施工單_SQL每日備份.gs`已部署，`設定施工單每日備份觸發器`已執行，咖哩實機執行確認Drive出現備份檔。
  - ⚠️ 踩坑記錄：第一次手動執行備份只抓到1000筆（實際4302筆），查出是Supabase/PostgREST單次請求預設上限1000筆，未分頁的GET會悄悄漏資料。加上limit+offset分頁抓取後修正，重跑確認完整抓到4303筆。**教訓：任何一次性GET讀取Supabase資料表，只要資料量可能超過1000筆，一律要做分頁，不能假設單次請求會拿到全部資料。**
  - 附帶處理：發現既有的`removeDuplicateBM`（Sheets端去重清理）有一個每分鐘執行一次的時間驅動觸發器，歷史錯誤率99.29%（就是先前修的getUi()問題），建議咖哩改成每天一次；這個跟SQL遷移本身無直接關係，是排查過程中順便發現的既有設定問題。
- [ ] 階段6（可選）：Sheets唯讀鏡像

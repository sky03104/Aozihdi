...
//    另外設計）。
//
// 查詢設計：長期施工授權（例如 6/1~6/30）用 work_date（起）跟 exit_date
// （迄）圈出涵蓋 dayA/dayB 的區間。用 work_date 索引先篩出「開始日期
// 不會太早」的範圍（180天內開始的），避免真的整表撈；exit_date 沒填的
// 用 work_date 當單日工程判斷（跟原本getOrders的fallback邏輯一致）。
// 180天是保守估計「不會有比這更長的施工期」，之後若真的遇到更長的案例
// 導致查不到，這個數字要跟著調大。
// ════════════════════════════════════════════════════════════

...
## 🆕 v2.0 升級步驟（已部署過 v1 的看這裡就好）

v2.0 新增：**即時航跡**（brain_map 顯示線上使用者 + 沿線移動動畫）、**強制登出**、CacheService 快取、讀取加速、清理加速。

1. 開啟「航海日誌」試算表 → 擴充功能 → Apps Script
2. **整份替換**成新的 `航海日誌_GAS.gs` 內容 → 儲存
3. **管理部署 → 編輯（鉛筆）→ 版本：新版本 → 部署**
   （⛔ 不要「新增部署」，URL 會變，前端全斷）
4. （強烈建議）設定主管名單：
   - 編輯器左側 **專案設定**（齒輪）→ 底部「指令碼屬性」→ 新增屬性
   - 屬性：`ADMIN_EMPIDS`，值：主管工號逗號分隔（例：`1001,1002`）
   - 設定後只有名單內工號能按「強制登出」；不設定則不限制
5. 測試：編輯器執行 `testForceLogoutAndKick()`，執行日誌應顯示
   kicked:true → kicked:false（一次性生效）
